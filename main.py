...
if not XAI_API_KEY:
    print("ОШИБКА: XAI_API_KEY не задан", flush=True)
    sys.exit(1)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        "Я бот с интеграцией Grok. Напиши мне или пришли картинку/мем — отвечу без занудства.\n\n"
        "Команды:\n"
        "/start — приветствие\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        "🤖 *Как пользоваться:*\n\n"
        "Пиши текстом или присылай фото/мемы — отвечу через Grok.\n\n"
        "Команды:\n"
        "/start — приветствие\n"
        "/clear — очистить историю чата\n"
        "/help — эта подсказка",
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await update.message.reply_text("⚠️ Ошибка при обращении к Grok. Попробуй позже.")
